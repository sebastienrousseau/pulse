...
class DashboardGenerator:
    ...
    def _generate_inline_dashboard(self, summary: EcosystemSummary) -> str:
        ...
        for repo in sorted(summary.repos, key=lambda r: -r.score):
            ...
            repo_rows.append(f"""
                <tr>
                    <td><a href="{html_mod.escape(repo.url)}" target="_blank" style="color: {text_color};">{html_mod.escape(repo.name)}</a></td>
                    <td><span style="color: {status_color};">●</span> {html_mod.escape(repo.status.value)}</td>
                    <td>{repo.score:.0f}</td>
                    <td>{html_mod.escape(repo.language.value)}</td>
                    <td>{html_mod.escape(repo.latest_build.value)}</td>
                    <td>{vulns}</td>
...
<body>
    <div class="container">
        ...
        <p class="subtitle">Ecosystem health monitoring for {html_mod.escape(summary.organization)}</p>
        ...
        <footer>
            Generated by Pulse at {datetime.now(tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}
        </footer>
    </div>
</body>
</html>"""
